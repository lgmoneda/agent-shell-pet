...
It is a separate package from =agent-shell=. It listens to the public
=agent-shell= event API, so it works with Codex, Claude, and other ACP-backed
agents.

* Features

- One Codex-like global pet by default.
...
- Uses the same =pet.json= and =spritesheet.webp= format as Codex.app.
- Ships with the =clawd= pet from [[https://codex-pets.net/#/pets/clawd][codex-pets.net]].
